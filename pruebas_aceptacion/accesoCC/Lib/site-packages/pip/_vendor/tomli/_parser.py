    ...
    *,
    ...
    if bad_chars:
        ...
        raise suffixed_err(
            src,
            bad_pos,
            f'Found invalid character "{bad_char!r}"')
    return new_pos
...
def create_dict_rule(src: str, pos: Pos, state: State) -> Pos:
    ...
    if state.flags.is_(key, Flags.EXPLICIT_NEST) or state.flags.is_(
            key, Flags.FROZEN):
...
def key_value_rule(src: str, pos: Pos, state: State,
    ...
    # after this
    state.flags.set_for_relative_key(
        state.header_namespace, key, Flags.EXPLICIT_NEST)
    try:
...
def parse_hex_char(src: str, pos: Pos, hex_len: int) -> Tuple[Pos, str]:
    ...
    if len(hex_str) != hex_len or any(
            c not in string.hexdigits for c in hex_str):
